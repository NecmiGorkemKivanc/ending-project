...
class ShopcartRepository extends ServiceEntityRepository
{
    ...
    */
    ...
    public function getUserShopCartCount($userid)
    {$b=0;
     $a=0;
        $em = $this->getEntityManager();
        $query = $em->createQuery('
        SELECT s.quantity as shopcount
    ...
    public function getUserMinPrice($userid)
    {
        $em = $this->getEntityManager();
        $query = $em->createQuery('
    ...
    {  
        $em = $this->getEntityManager();
        $query = $em->createQuery('
        SELECT p, c as pcat FROM App\Entity\Product p
            JOIN p.category c , App\Entity\Shopcart s
            WHERE  s.productid = p.id and s.userid=:userid')
            ->setParameter('userid', $userid);
